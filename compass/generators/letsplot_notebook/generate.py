#!/usr/bin/env python3
"""LetsPlot notebook generator -- generates Jupyter notebooks with LetsPlot charts.

G_letsplot  : Query -> LetsPlotNotebookSpec
V_letsplot  : syntax check + exec all code cells (generates charts)
...
from ._context import build_letsplot_context
...
def _make_invoke(base_url: str = "", ask_fn=None):
    """G_letsplot: Context -> Result[LetsPlotNotebookSpec]"""
...
def _make_parse():
    """V1: LetsPlotNotebookSpec -> Result[LetsPlotNotebookSpec]"""
    def parse(spec: LetsPlotNotebookSpec) -> Result:
        return validate_spec_instance(spec)
    return parse


def _make_validate():
    """V2: LetsPlotNotebookSpec -> Result[LetsPlotNotebookSpec]"""
    def validate(spec: LetsPlotNotebookSpec) -> Result:
        return validate_notebook(spec)
    return validate


def _make_fix(base_url: str = "", ask_fn=None):
    """G'_letsplot: (spec, error, ctx) -> spec | None"""
    def fix(spec: LetsPlotNotebookSpec, error: str, ctx: GenerationContext):
        return ouroboros_notebook(spec, error, ctx, base_url, ask_fn)
    return fix
...
def run(
    ...
    *,
    ...
    logger.info("Building LetsPlot notebook generation context...")
    ctx = build_letsplot_context(prompt)
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="LetsPlot notebook generator",
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )
    parser.add_argument(
        "--prompt", type=str, help="What notebook to generate"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Path to existing notebook to refine",
    )
    parser.add_argument(
    ...
    if args.live:
        ...
        return result_to_exit(repl_loop(
            ...
            emit=_make_emit(Path(args.output_dir or "notebooks")),
            max_rounds=args.max_rounds,
            max_fixes=args.max_fixes,
            load_artifact=load_notebook,
            banner="LetsPlot Notebook Generator -- interactive mode",
        ))
